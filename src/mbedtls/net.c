...
#include <sys/types.h>
#include <lwip/lwip/sockets.h>	// Changed by Dayer from <sys/socket.h>
//#include <netinet/in.h>		// Removed by Dayer
#include <lwip/lwip/inet.h>		// Changed by Dayer from <arpa/inet.h>
...
#include <signal.h>
#include <fcntl.h>
#include <lwip/lwip/netdb.h>						// Changed by Dayer from <netdb.h>
#include <errno.h>
...
#else
// #include <endian.h>		// Removed by Dayer
#endif
...
int net_bind( int *fd, const char *bind_ip, int port )
{
    ...
    hints.ai_socktype = SOCK_STREAM;
    hints.ai_protocol = IPPROTO_TCP;
    //if( bind_ip == NULL )				// Removed by Dayer
    //    hints.ai_flags = AI_PASSIVE;	// Removed by Dayer
...
static int net_would_block( int fd )
{
     ...
     */
    if( ( fcntl( fd, F_GETFL, 0 ) & O_NONBLOCK ) != O_NONBLOCK )	// Changed by Dayer. (Added ", 0" for fcntl)
        return( 0 );
...
int net_accept( int bind_fd, int *client_fd, void *client_ip )
{
    ...
    *client_fd = (int) accept( bind_fd, (struct sockaddr *) &client_addr, (socklen_t*)&n );	// Dayer added (socklen_t*) for pointer type compatibility
...
#else
    return( fcntl( fd, F_SETFL, fcntl( fd, F_GETFL, 0 ) & ~O_NONBLOCK ) );	// Dayer Added ", 0" in fcntl. (lwip's fcntl takes 3 args, even for GETFL)
#endif
}
